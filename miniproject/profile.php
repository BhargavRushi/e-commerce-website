...
<form action = "db.php" method = "POST" enctype="multipart/form-data" class = "text-info">
	...
	<div class = "row">
		<div class = "col-lg-3">
			...
			if(empty($row['image']))
			{
				echo "<button type = 'submit' name = 'mpimg' class='btn btn-primary'>Upload image</button>";
			}
			else
			{
				echo"<button type = 'submit' name = 'mpimg' class='btn btn-primary'>Change image</button>";
			}
		?>
		</div>
	<div class = "col-lg-9">
		...
		<a href = "db.php?edit" class="btn btn-success" name = "edit">Edit</a>
		<a href='db.php?changepassword' class="btn btn-danger" name='changepassword'>Change Password</a>
	</div>
</form>
</div>
</div>
